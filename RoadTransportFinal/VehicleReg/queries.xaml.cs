...
namespace RoadTransportFinal.vehiclereg
{
    ...
    {
        List<string> mylist = new List<string>() {  "Status of Application", "Payment Receipts", "Near-by-RTOS" ,"Check Vehicle Details" };
        public queries()
        {
        ...
        private void listBox_SelectionChanged_2(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (item.Equals("Status of Application"))
            {
                Frame.Navigate(typeof(VehicleReg.Sub_Queries.CAS));
            }
            else if (item.Equals("Payment Receipts"))
            {
                Frame.Navigate(typeof(VehicleReg.Sub_Queries.Recipts));
            }
            else if (item.Equals("Near-by-RTOS"))
            {
                Frame.Navigate(typeof(VehicleReg.Sub_Queries.NR));
            }
            else if (item.Equals("Check Vehicle Details"))
            {
                Frame.Navigate(typeof(VehicleReg.CheckDetails));
            }
            else
            {
