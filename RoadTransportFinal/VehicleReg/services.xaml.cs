...
namespace RoadTransportFinal.vehiclereg
{
    ...
    {
        List<string> mylist = new List<string> { "Registration", "Change of Address","Renewal","No Objection Certificate(NOCs)","Slot Booking" };
        public services()
        {
        ...
        private void listBox_SelectionChanged_2(object sender, SelectionChangedEventArgs e)
        {
            ...
            if (item.Equals("Registration"))
            {
                Frame.Navigate(typeof(Forms.TempReg.Pretempreg));
            }
           
            else if (item.Equals("Change of Address"))
            {
                Frame.Navigate(typeof(Forms.changeofaddress));
            }
            else if (item.Equals("Renewal"))
            {
                Frame.Navigate(typeof(Forms.Regrenewal));
            }
            else if (item.Equals("No Objection Certificate(NOCs)"))
            {
                Frame.Navigate(typeof(VehicleReg.Sub_service.noc));
            }
            else if (item.Equals("Slot Booking"))
            {
                Frame.Navigate(typeof(VehicleReg.Sub_service.slot));
            }
